...
PROMPT_VERSION = "v5-use-case-v1"
MAX_USE_CASE_VALUES = 5
_EXPECTED_RESPONSE_FIELDS = {"use_case"}

USE_CASE_PROMPT = """You are annotating exactly one shopping-catalog product for one field: use_case.
...
- Include only clear, supported positive activities, environments, situations,
  or purposes for using the product. Examples include running, hiking, gym,
  office, wedding, travel, cosplay, sleepwear, and everyday wear when the row
  genuinely supports them.
- Do not invent a use case from a vague product category or demographic label.
- Warnings, prohibitions, cleaning/storage instructions, package contents, and
  unrelated accessories are not positive product use cases. For example,
  "avoid wearing while swimming" does not support "swimming".
- Keep the list concise and deduplicated. Return at most five values.
- Values must be lowercase natural-language phrases with spaces, not snake_case.
- Return [] when no clear use case is supported.
...
def build_use_case_prompt(
    ...
    *,
    ...
    prompt = USE_CASE_PROMPT + row
    if retry_instruction:
        prompt += "\n\nCORRECTION: " + retry_instruction
    return prompt


def _normalize_use_case_value(value: str) -> str:
    if not isinstance(value, str):
        raise TypeError("use_case values must be strings")
    ...
    if not normalized:
        raise ValueError("use_case values must not be empty")
    return normalized


def parse_and_validate_use_case(raw_response: Any) -> dict[str, list[str]]:
    ...
    if set(payload) != _EXPECTED_RESPONSE_FIELDS:
        raise ValueError("model response must contain exactly the use_case key")
    values = payload["use_case"]
    if not isinstance(values, list):
        raise TypeError("use_case must be an array of strings")
    if len(values) > MAX_USE_CASE_VALUES:
        raise ValueError(f"use_case must contain at most {MAX_USE_CASE_VALUES} values")
    normalized_values: list[str] = []
    seen: set[str] = set()
    for value in values:
        normalized = _normalize_use_case_value(value)
        if normalized in seen:
            raise ValueError("use_case values must be deduplicated")
        seen.add(normalized)
        normalized_values.append(normalized)
    return {"use_case": normalized_values}


def validate_use_case_record(record: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(record, Mapping):
        raise TypeError("use_case record must be an object")
    if set(record) != {"parent_asin", "use_case"}:
        raise ValueError("use_case record must contain exactly parent_asin and use_case")
    parent_asin = record["parent_asin"]
    if not isinstance(parent_asin, str) or not parent_asin.strip():
        raise ValueError("use_case record needs a non-empty parent_asin")
    parsed = parse_and_validate_use_case({"use_case": record["use_case"]})
    return {"parent_asin": parent_asin, "use_case": parsed["use_case"]}
...
def _read_successes(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            record = validate_use_case_record(raw)
            parent_asin = record["parent_asin"]
        ...
        try:
            ...
            if previous_retry_kind == "schema":
                retry_instruction = (
                    ...
                    "JSON object containing only the use_case array, with no "
                    "extra keys or prose."
                )
            raw_response = client.annotate(
                build_use_case_prompt(product, retry_instruction=retry_instruction)
            )
            parsed = parse_and_validate_use_case(raw_response)
            record = validate_use_case_record(
                {"parent_asin": product["parent_asin"], "use_case": parsed["use_case"]}
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume-safe use_case-only catalog annotation runner."
    )
    parser.add_argument("--catalog", default="data/catalog.jsonl")
    parser.add_argument(
        "--output",
        default="data/derived/annotations/v5/use_case.jsonl",
    )
    parser.add_argument(
        "--env-file",
    ...
    model = args.model or os.environ.get("ANNOTATION_MODEL", "catalog-annotator-v5-use-case")
    ...
    summary = run_use_case_annotation(
        args.catalog,
        args.output,
        client,
        model=model,

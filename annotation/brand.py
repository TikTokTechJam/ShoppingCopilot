...
PROMPT_VERSION = "v5-brand-v1"
MAX_BRAND_VALUES = 2
_EXPECTED_RESPONSE_FIELDS = {"brand"}

BRAND_PROMPT = """You are annotating exactly one shopping-catalog product for one field: brand.
...
Rules:
- Identify the actual brand, maker, label, or clearly established product-line
  identity of the MAIN PRODUCT.
- Use the entire supplied row, including title, features, description, details,
  categories, store, and every other field, as evidence.
- Return the consumer-facing company/brand when it is clearly supported.
- You may also return one concise model or product-line identity when the row
  clearly ties it to the main product. A company and model line may therefore
  appear together, such as ["nike", "air max 270"] or ["crocs", "classic clog"].
- Do not return seller or store names, unrelated manufacturer names, compatible
  brands, comparison brands, bundled-item brands, random capitalized words,
  category names, styles, materials, colors, or use cases.
- Do not infer a brand from weak evidence. If the main-product brand is unclear,
  return [].
- Do not return generic descriptions as product-line identity, such as
  "comfortable running shoe" or "black sneakers".
- Keep the list concise, deduplicated, and limited to at most two values.
- Values must be lowercase natural-language text with spaces, not snake_case.
  Preserve meaningful punctuation where appropriate, including apostrophes and
  periods in names such as "levi's" and "dr. martens".
- Normalize obvious formatting variants consistently, for example
  "New_Balance" and "NEW BALANCE" become "new balance".
...
def build_brand_prompt(
    ...
    *,
    ...
    prompt = BRAND_PROMPT + row
    if retry_instruction:
        prompt += "\n\nCORRECTION: " + retry_instruction
    return prompt


def _normalize_brand_value(value: str) -> str:
    if not isinstance(value, str):
        raise TypeError("brand values must be strings")
    ...
    if not normalized or not any(character.isalnum() for character in normalized):
        raise ValueError("brand values must contain a usable name")
    return normalized


def parse_and_validate_brand(raw_response: Any) -> dict[str, list[str]]:
    ...
    if set(payload) != _EXPECTED_RESPONSE_FIELDS:
        raise ValueError("model response must contain exactly the brand key")
    values = payload["brand"]
    if not isinstance(values, list):
        raise TypeError("brand must be an array of strings")
    if len(values) > MAX_BRAND_VALUES:
        raise ValueError(f"brand must contain at most {MAX_BRAND_VALUES} values")
    normalized_values: list[str] = []
    seen: set[str] = set()
    for value in values:
        normalized = _normalize_brand_value(value)
        if normalized in seen:
            raise ValueError("brand values must be deduplicated")
        seen.add(normalized)
        normalized_values.append(normalized)
    return {"brand": normalized_values}


def validate_brand_record(record: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(record, Mapping):
        raise TypeError("brand record must be an object")
    if set(record) != {"parent_asin", "brand"}:
        raise ValueError("brand record must contain exactly parent_asin and brand")
    parent_asin = record["parent_asin"]
    if not isinstance(parent_asin, str) or not parent_asin.strip():
        raise ValueError("brand record needs a non-empty parent_asin")
    parsed = parse_and_validate_brand({"brand": record["brand"]})
    return {"parent_asin": parent_asin, "brand": parsed["brand"]}
...
def _read_successes(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            record = validate_brand_record(raw)
            parent_asin = record["parent_asin"]
        ...
        try:
            ...
            if previous_retry_kind == "schema":
                retry_instruction = (
                    ...
                    "JSON object containing only the brand array, with no "
                    "extra keys or prose."
                )
            raw_response = client.annotate(
                build_brand_prompt(product, retry_instruction=retry_instruction)
            )
            parsed = parse_and_validate_brand(raw_response)
            record = validate_brand_record(
                {"parent_asin": product["parent_asin"], "brand": parsed["brand"]}
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume-safe brand-only catalog annotation runner."
    )
    parser.add_argument("--catalog", default="data/catalog.jsonl")
    parser.add_argument(
        "--output",
        default="data/derived/annotations/v5/brand.jsonl",
    )
    parser.add_argument(
        "--env-file",
    ...
    model = args.model or os.environ.get("ANNOTATION_MODEL", "catalog-annotator-v5-brand")

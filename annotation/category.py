...
PROMPT_VERSION = "v5-category-v1"
MAX_CATEGORY_VALUES = 3
_EXPECTED_RESPONSE_FIELDS = {"category"}

CATEGORY_PROMPT = """You are annotating exactly one shopping-catalog product for one field: category.
...
Do NOT output STYLE / SHAPE / APPEARANCE:
- long sleeve
- short sleeve
- v neck
- high waist
...
Do NOT output CATEGORY / PRODUCT TYPE:
- sandals
- hoodie
- socks
- necklace

Do NOT output USE CASE:
- running
- hiking
- office
- wedding
...
- Use title, features, description, details, existing taxonomy, and every other
  supplied field as evidence, while ignoring text that describes other items.
- Use short, concrete, human-readable object nouns such as shoes, running shoes,
  boots, hiking boots, socks, ankle socks, dress, jacket, necklace, earrings,
  bracelet, backpack, handbag, belt, or watch when supported.
- A specific type may coexist with one useful parent object, such as
  ["shoes", "running shoes"] or ["jewelry", "necklace"]. Return only one to
  three useful levels; do not produce redundant near-duplicates.
- Do not copy taxonomy or navigation labels such as men, women, boys, girls,
  clothing, accessories, casual, active, fashion, novelty, department names,
  audience/gender labels, style, material, or use case.
- Do not classify accessories, packaging, bundled secondary items, compatible
  products, or unrelated products mentioned in text.
- Prefer common product naming and lowercase natural-language phrases with
  spaces, not snake_case.
- Return [] only when the main product type genuinely cannot be determined.
- Do not invent unsupported product types.
...
def _normalize_category_value(value: str) -> str:
    if not isinstance(value, str):
        raise TypeError("category values must be strings")
...
def parse_and_validate_category(raw_response: Any) -> dict[str, list[str]]:
    ...
    if set(payload) != _EXPECTED_RESPONSE_FIELDS:
        raise ValueError("model response must contain exactly the category key")
    values = payload["category"]
    if not isinstance(values, list):
        raise TypeError("category must be an array of strings")
    if len(values) > MAX_CATEGORY_VALUES:
        raise ValueError(f"category must contain at most {MAX_CATEGORY_VALUES} values")
    normalized_values: list[str] = []
    seen: set[str] = set()
    for value in values:
        normalized = _normalize_category_value(value)
        if normalized in seen:
            raise ValueError("category values must be deduplicated")
        seen.add(normalized)
        normalized_values.append(normalized)
    return {"category": normalized_values}


def validate_category_record(record: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(record, Mapping):
        raise TypeError("category record must be an object")
    if set(record) != {"parent_asin", "category"}:
        raise ValueError("category record must contain exactly parent_asin and category")
    parent_asin = record["parent_asin"]
    if not isinstance(parent_asin, str) or not parent_asin.strip():
        raise ValueError("category record needs a non-empty parent_asin")
    parsed = parse_and_validate_category({"category": record["category"]})
    return {"parent_asin": parent_asin, "category": parsed["category"]}
...
def _read_successes(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            record = validate_category_record(raw)
            parent_asin = record["parent_asin"]
        ...
        try:
            ...
            if previous_retry_kind == "schema":
                retry_instruction = (
                    ...
                    "JSON object containing only the category array, with no "
                    "extra keys or prose."
                )
            raw_response = client.annotate(
                build_category_prompt(product, retry_instruction=retry_instruction)
            )
            parsed = parse_and_validate_category(raw_response)
            record = validate_category_record(
                {"parent_asin": product["parent_asin"], "category": parsed["category"]}
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume-safe category-only catalog annotation runner."
    )
    parser.add_argument("--catalog", default="data/catalog.jsonl")
    parser.add_argument(
        "--output",
        default="data/derived/annotations/v5/category.jsonl",
    )
    parser.add_argument(
        "--env-file",
    ...
    model = args.model or os.environ.get("ANNOTATION_MODEL", "catalog-annotator-v5-category")
    ...
    summary = run_category_annotation(
        args.catalog,
        args.output,
        client,
        model=model,

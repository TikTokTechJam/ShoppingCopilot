...
COLOR_PROMPT = """You are annotating exactly one shopping-catalog product for one field: color.
...
Rules:
- Identify the actual visible color or colors of the MAIN PRODUCT.
...
- Include colors such as black, white, navy, blue, red, green, beige, brown,
  gold, silver, pink, purple, multicolor, transparent, or clear only when the
  row clearly supports the main product having that visible color.
- Extract only colors belonging to the main product. Do not use colors of
  packaging, bundled secondary items, accessories, decorative props, models,
  people, backgrounds, compatible products, comparison products, or unrelated
  items mentioned in text.
- Do not infer a color from a material alone. For example, gold plated supports
  gold only when the product is actually gold-colored; sterling silver may
  support silver when it describes the visible product color.
- Keep the list concise, deduplicated, and limited to at most three colors.
- Use lowercase natural-language phrases with spaces, not snake_case. Preserve
  meaningful phrases such as rose gold and off white.
- Normalize obvious color variants consistently: use gray rather than grey,
  navy rather than navy blue when equivalent, gold rather than golden, and
  multicolor rather than multi color.
- Do not output brand, material, style, category, or use-case values.
- Do not invent colors from the product type, brand, material, or texture.
- Return [] when no clear main-product color is supported.
...
def _normalize_color_value(value: str) -> str:
    ...
    if not normalized:
        raise ValueError("color values must not be empty")
    return COLOR_ALIASES.get(normalized, normalized)


def parse_and_validate_color(raw_response: Any) -> dict[str, list[str]]:
    ...
    if set(payload) != _EXPECTED_RESPONSE_FIELDS:
        raise ValueError("model response must contain exactly the color key")
    ...
    for value in values:
        ...
        if normalized in seen:
            raise ValueError("color values must be deduplicated")
        seen.add(normalized)
        normalized_values.append(normalized)
    return {"color": normalized_values}


def validate_color_record(record: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(record, Mapping):
        raise TypeError("color record must be an object")
    if set(record) != {"parent_asin", "color"}:
        raise ValueError("color record must contain exactly parent_asin and color")
    parent_asin = record["parent_asin"]
    if not isinstance(parent_asin, str) or not parent_asin.strip():
        raise ValueError("color record needs a non-empty parent_asin")
    parsed = parse_and_validate_color({"color": record["color"]})
    return {"parent_asin": parent_asin, "color": parsed["color"]}
...
def _read_successes(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            record = validate_color_record(raw)
            parent_asin = record["parent_asin"]
        ...
        try:
            ...
            if previous_retry_kind == "schema":
                retry_instruction = (
                    ...
                    "JSON object containing only the color array, with no "
                    "extra keys or prose."
                )
            raw_response = client.annotate(
                build_color_prompt(product, retry_instruction=retry_instruction)
            )
            parsed = parse_and_validate_color(raw_response)
            record = validate_color_record(
                {"parent_asin": product["parent_asin"], "color": parsed["color"]}
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume-safe color-only catalog annotation runner."
    )
    parser.add_argument("--catalog", default="data/catalog.jsonl")
    parser.add_argument(
        "--output",
        default="data/derived/annotations/v5/color.jsonl",
    )
    parser.add_argument(
        "--env-file",
    ...
    model = args.model or os.environ.get("ANNOTATION_MODEL", "catalog-annotator-v5-color")

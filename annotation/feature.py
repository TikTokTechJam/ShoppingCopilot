...
FEATURE_PROMPT = """You are annotating exactly one shopping-catalog product for one field: feature.
...
- Include only intrinsic properties, capabilities, or design characteristics
  of the main product itself. Valid concepts include waterproof, moisture
  wicking, zipper closure, pockets, arch support, machine washable, adjustable,
  non slip, lightweight, breathable, reflective, and removable straps when
  grounded in the row.
- Do not invent a feature from a vague product category or demographic label.
- Do not extract facts belonging only to accessories, bundled secondary items,
  packaging, unrelated products, compatible devices/items, example outfits,
  comparison products, or other text that does not describe the main product.
- Do not return use cases such as running, wedding, or office as features.
- Do not return materials such as polyester, leather, or cotton as features.
- Avoid style labels unless they clearly describe an actual product feature.
- Warnings, prohibitions, cleaning/storage instructions, package contents, and
  unrelated accessories are not product features.
- Keep the list concise and deduplicated. Return at most eight values.
- Values must be lowercase natural-language phrases with spaces, not snake_case.
- Return [] when no clear feature of the main product is supported.
...
def parse_and_validate_feature(raw_response: Any) -> dict[str, list[str]]:
    ...
    if set(payload) != _EXPECTED_RESPONSE_FIELDS:
        raise ValueError("model response must contain exactly the feature key")
    values = payload["feature"]
    if not isinstance(values, list):
        raise TypeError("feature must be an array of strings")
    if len(values) > MAX_FEATURE_VALUES:
        raise ValueError(f"feature must contain at most {MAX_FEATURE_VALUES} values")
    normalized_values: list[str] = []
    seen: set[str] = set()
    for value in values:
        normalized = _normalize_feature_value(value)
        if normalized in seen:
            raise ValueError("feature values must be deduplicated")
        seen.add(normalized)
        normalized_values.append(normalized)
    return {"feature": normalized_values}


def validate_feature_record(record: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(record, Mapping):
        raise TypeError("feature record must be an object")
    if set(record) != {"parent_asin", "feature"}:
        raise ValueError("feature record must contain exactly parent_asin and feature")
    parent_asin = record["parent_asin"]
    if not isinstance(parent_asin, str) or not parent_asin.strip():
        raise ValueError("feature record needs a non-empty parent_asin")
    parsed = parse_and_validate_feature({"feature": record["feature"]})
    return {"parent_asin": parent_asin, "feature": parsed["feature"]}
...
def _read_successes(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            record = validate_feature_record(raw)
            parent_asin = record["parent_asin"]
        ...
        try:
            ...
            if previous_retry_kind == "schema":
                retry_instruction = (
                    ...
                    "JSON object containing only the feature array, with no "
                    "extra keys or prose."
                )
            raw_response = client.annotate(
                build_feature_prompt(product, retry_instruction=retry_instruction)
            )
            parsed = parse_and_validate_feature(raw_response)
            record = validate_feature_record(
                {"parent_asin": product["parent_asin"], "feature": parsed["feature"]}
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume-safe feature-only catalog annotation runner."
    )
    parser.add_argument("--catalog", default="data/catalog.jsonl")
    parser.add_argument(
        "--output",
        default="data/derived/annotations/v5/feature.jsonl",
    )
    parser.add_argument(
        "--env-file",
    ...
    model = args.model or os.environ.get("ANNOTATION_MODEL", "catalog-annotator-v5-feature")
    ...
    summary = run_feature_annotation(
        args.catalog,
        args.output,
        client,
        model=model,
